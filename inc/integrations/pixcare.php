 ...
 */
function lens_force_redirect_to_pixcare_install_once() {
...
add_action( 'after_switch_theme', 'lens_force_redirect_to_pixcare_install_once' );

/**
 *  * Redirect the admin to the pixcare install page once only if the plugin is missing
 */
function lens_admin_redirect_to_pixcare_install_once() {
...
add_action( 'admin_init', 'lens_admin_redirect_to_pixcare_install_once' );

function lens_add_pixcare_install_page() {
	add_submenu_page( null, '', '', 'manage_options', 'pixelgrade_care-install', null );
}
add_action( 'admin_menu', 'lens_add_pixcare_install_page' );

function lens_pixcare_install_page() {
	...
	<head>
		...
		<title><?php esc_html_e( 'Pixelgrade Care &rsaquo; Installer', 'lens' ); ?></title>
			...
			<div class="stepper">
				<div class="stepper__content">
					...
					<div class="section section--informative">
						...
						<h2 class="section__title u-text-center"><?php esc_html_e( 'One more lap to go..', 'lens' ); ?></h2>
						<div class="section__content">
							<?php
							echo sprintf(
								'%s <strong>%s</strong> %s',
								esc_html__( 'Start your digital journey with our', 'lens' ),
								esc_html__( 'Pixelgrade Care®', 'lens' ),
								esc_html__( 'on board. It\'s the core of your experience with our WordPress theme, and we bet you are going to love it from day one. You’re in great hands, no doubt about that!', 'lens' )
							); ?>
						</div>
						<div class="plugins">
							<div class="plugin  box  box--neutral  box--plugin-missing  box--plugin-invalidated">
								<div class="box__body">
									<h5 class="box__title"><?php esc_html_e( 'Pixelgrade Care®', 'lens' ) ?></h5>
									<p class="box__text"><?php esc_html_e( 'Installing ...', 'lens' ) ?></p>
								</div>
							</div>
						</div>
					</div>
					...
					<a class="btn  btn--text  btn--return-to-dashboard"
					   href="<?php echo esc_url( admin_url() ); ?>"><?php esc_html_e( 'Return to the WordPress Dashboard', 'lens' ) ?></a>
				</div>
			</div>
		</div>
	</div>
        ...
        (function ($) {
            $(document).ready(function () {
                ...
                wp.ajax.send({type: 'GET'}).always(function (res) {
                    var success = res.indexOf('<p><?php esc_html_e( 'Plugin installed successfully.', 'lens' ); ?></p>');
                    var already_exists = res.indexOf('<p><?php esc_html_e( 'Plugin destination folder already exists.', 'lens' ); ?></p>');
                    ...
                    if (success !== -1) {
                        ...
                        $status.text('<?php esc_html_e( 'Bummer! Something went wrong. I’m sorry but you will need to install the plugin manually. Redirecting you to the appropriate page…', 'lens' ); ?>');
...
add_action( 'admin_init', 'lens_pixcare_install_page' );
